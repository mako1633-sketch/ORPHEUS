...
describe("security control mapping", () => {
	it("maps Windows findings to control statuses and insurance unknowns", () => {
		const findings = [
			finding("defender-realtime-disabled", "Defender real-time protection appears disabled", "high"),
		...
		expect(mappings.find((mapping) => mapping.id === "cis-10-malware-defenses")?.status).toBe("failed");
		expect(mappings.find((mapping) => mapping.id === "insurance-mfa")?.status).toBe("unknown");
		expect(mappings.some((mapping) => mapping.relatedFindingIds.includes("admin-membership-review"))).toBe(
			true
		);
	});
});
