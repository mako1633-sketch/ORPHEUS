...
import { runDirectDaemonDoctor, shouldRunDirectDaemonDoctor } from "../src/ai/direct-daemon-doctor-runner";
...
describe("direct ORPHEUS doctor runner", () => {
	it("routes doctor setup prompts to deterministic status execution", () => {
		expect(shouldRunDirectDaemonDoctor("Run ORPHEUS doctor: keys, tools, PowerShell, Signal, search")).toBe(
			true
		);
		expect(shouldRunDirectDaemonDoctor("Run DAEMON doctor: keys, tools, PowerShell, Signal, search")).toBe(
			true
		);
