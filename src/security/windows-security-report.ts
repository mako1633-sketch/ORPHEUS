...
export type SecurityReportActionStage =
	| "Do now"
	| "Needs admin"
...
export function categoryForFinding(
	finding: Pick<AssessmentFinding, "id" | "title">
...
function actionStageForFinding(finding: AssessmentFinding): SecurityReportActionStage {
	...
	if (
		/admin|firewall|defender|protection|policy/i.test(`${finding.title} ${finding.remediation}`)
	) {
		return "Needs admin";
	}
	if (
		/inbound|administrator|remove|disable|rdp|service/i.test(
...
function buildBreakdown(findings: SecurityReportFinding[]): SecurityScoreBreakdown[] {
	return ALL_CATEGORIES.map((category) => {
		...
		const actionableCount = categoryFindings.filter(
...
function buildPromptSuggestions(): PromptSuggestion[] {
	return [
		...
		},
		{
			...
			prompt:
				"Map this report to cyber insurance readiness controls and list unanswered questions.",
		},
		{
			label: "Export report",
...
function buildReportMarkdown(report: Omit<WindowsSecurityReport, "markdown">): string {
	...
	const actionLines =
		report.actions.length > 0
			? report.actions
					.map((action) => `- **${action.stage}:** ${action.title} - ${action.action}`)
					.join("\n")
...
export function buildWindowsSecurityReport(
	...
	const penalty = reportFindings.reduce(
		(sum, finding) => sum + SEVERITY_WEIGHTS[finding.severity],
		0
	);
	const score = clampScore(100 - penalty);
